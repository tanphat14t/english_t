<template>
    <div>
        <div class="chat_view_list">
                ...
                <div class="main-title">
                    <div class="dropdown CRM_dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu3">
                            <a v-if="my_role == 1" data-toggle="modal" data-target="#manageRole" class="dropdown-item"
                               type="button">
                                {{ __('chat.user_role') }}
                            </a>
                            ...
                            <a v-if="can_add_people" data-toggle="modal" data-target="#addPeopleForm"
                               class="dropdown-item" type="button">
                                {{ __('chat.add') }} {{ __('chat.people') }}
                            </a>
                            <a data-toggle="modal" data-target="#removePeopleForm" class="dropdown-item" type="button">
                                {{ __('chat.member') }}
                            </a>
                            <a :href="files_url" class="dropdown-item" type="button">
                                {{ __('chat.media_files') }}
                            </a>
                            <a @click="leave_group()" class="dropdown-item" type="button">
                                {{ __('chat.leave') }} {{ __('chat.group') }}
                            </a>
                            <a v-if="my_role == 1" @click="delete_group()" class="dropdown-item" type="button">
                                {{ __('chat.delete') }} {{ __('chat.group') }}
                            </a>
                            <a v-if="my_role == 1 && !this.read_only" @click="make_read_only()" class="dropdown-item"
                               type="button">
                                {{ __('chat.mark_as_read_only') }}
                            </a>

                            <a v-if="my_role == 1 && this.read_only" @click="make_read_only('unmark')"
                               class="dropdown-item" type="button">
                                {{ __('chat.remove_read_only') }}
                            </a>
                        </div>
                    </div>
                </div>
                <StatusChangeComponent :app_url="this.app_url" :user="this.user"
            ...
            <div class="chat_view_list_inner crm_full_height">
                <div v-if="search_result_bar" class="search_indicator py-2">
                    <div class="float-left">
                        <p class="mb-0">{{ __('chat.showing') }} {{ this.search_index + 1 }} {{ __('chat.of') }}
                            {{ this.search_result_count }} {{ __('chat.results') }}</p>
                    </div>
                ...
                <div class="chat_view_list_inner_scrolled" id="chat_container" style="max-height: 70vh;" v-chat-scroll>
                    <div>
                        <p class="text-center" v-if="loadable">
                            <a class="cursor-pointer" @click="loadMore()"> {{ __('chat.load_more') }} <span
                                class="ti-arrow-up"></span> </a>
                        </p>
                        <div :id="'target'+index" v-for="(thread, index) in this.only_threads">
                            <div v-if="parseInt(thread.user.id) != parseInt(user.id) && !thread.removedByMe"
                                 class="chat_single d-flex" style="overflow: unset;">
                                <div class="thumb mr_20">
                                    <a v-if="thread.user.avatar" href="#"><img :src="baseUrl+ thread.user.avatar"
                                                                               alt=""/></a>
                                    <a v-else-if="thread.user.avatar_url" href="#"><img
                                        :src="baseUrl+asset_type+ thread.user.avatar_url" alt=""/></a>
                                    <a v-else href="#"><img :src="baseUrl+asset_type+'/chat/images/spondon-icon.png'"
                                                            alt=""/></a>
                                </div>
                                <div class="chat_text_info_wraper d-flex align-items-center">
                                    <div class="chat_text_info">
                                        <div v-if="thread.conversation.reply">
                                            <p class="reply_p" v-if="thread.conversation.reply.message_type == 0">
                                                {{ thread.conversation.reply.message }}</p>
                                            <p class="reply_p" v-else>{{ __('chat.attachment') }}</p>
                                        </div>

                                        <div v-if="thread.conversation.forward_from">
                                            <p class="reply_p font-italic"
                                               v-if="thread.conversation.forward_from.message_type == 0">
                                                <span>{{ __('chat.forwarded_message') }} : </span><br/>
                                                {{ thread.conversation.forward_from.message }}
                                            </p>
                                            <div class="reply_p audio-padding"
                                                 v-if="thread.conversation.forward_from.message_type == 4">
                                                <span>{{ __('chat.forwarded_message') }} : </span><br/>
                                                <audio
                                                    :src="baseUrl+asset_type+'/'+thread.conversation.forward_from.file_name"
                                                    controls></audio>
                                            </div>
                                            <div v-else-if="thread.conversation.forward_from.message_type == 5"
                                            ...
                                            <div class="reply_p p-3"
                                                 v-else-if="thread.conversation.forward_from.message_type == 1">
                                                <span>{{ __('chat.forwarded_message') }} : </span><br/>
                                                <img
                                                    class="border-radius-25 cursor-pointer"
                                                    @click="imageViewLargeScreen(baseUrl+thread.conversation.forward_from.file_name)"
                                                    v-if="thread.conversation.forward_from.file_name"
                                                    :src="baseUrl+thread.conversation.forward_from.file_name"
                                                    alt=""
                                                />
                                            ...
                                            <p class="reply_p"
                                               v-else-if="thread.conversation.message_type == 2 || thread.conversation.message_type == 3">
                                                <span>{{ __('chat.forwarded_message') }} : </span><br/>
                                                <u><a style="color: white;"
                                                      :href="app_url+'/chat/file/download/'+thread.conversation.forward_from.id">{{ thread.conversation.forward_from.original_file_name }}</a></u>
                                            </p>
                                        </div>

                                        <div class="audio-padding" v-if="thread.conversation.message_type == 4">
                                            <audio :src="baseUrl+asset_type+'/'+thread.conversation.file_name"
                                                   controls></audio>
                                        </div>
                                        <div v-else-if="thread.conversation.message_type == 5" class="p-3 w-100">
                                            <video class="w-100 border-radius-25" controls>
                                                <source :src="baseUrl+asset_type+'/'+thread.conversation.file_name"
                                                        type="video/mp4"/>
                                            </video>
                                        </div>
                                        <p v-else-if="thread.conversation.message_type == 0">
                                            <b>{{ thread.user.first_name }} {{ thread.user.last_name }} : </b>
                                            <br/>
                                            <span :id="'text'+index" class="textmsg"
                                                  v-html="url_maker(thread.conversation.message)"></span>
                                        ...
                                        <div class="p-3" v-else-if="thread.conversation.message_type == 1">
                                            <img
                                                class="border-radius-25 cursor-pointer"
                                                @click="imageViewLargeScreen(baseUrl+thread.conversation.file_name)"
                                                v-if="thread.conversation.file_name"
                                                :src="baseUrl+thread.conversation.file_name"
                                                alt=""
                                            />
                                            <img class="border-radius-25" v-else
                                                 :src="baseUrl+asset_type+'/chat/images/msg_img.png'" alt=""/>
                                        ...
                                        <p v-else-if="thread.conversation.message_type == 2 || thread.conversation.message_type == 3">
                                            <u><a style="color: white;"
                                                  :href="app_url+'/chat/file/download/'+thread.conversation.id">{{ thread.conversation.original_file_name }}</a></u>
                                        </p>
                                    ...
                                    <span class="chat_date ml_15 ml-2">
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right"
                                                ...
                                                <a @click="deleteMessage(thread)" class="dropdown-item cursor-pointer">
                                                    {{ __('chat.delete') }}
                                                </a>
                                            </div>
                                        </div>
                            ...
                            <div v-if="thread.user.id == user.id && !thread.removedByMe"
                                 class="chat_single d-flex sender_chat" style="overflow: unset;">
                                <div class="chat_text_info_wraper d-flex align-items-center">
                                    ...
                                    <div class="chat_text_info">
                                        <div v-if="thread.conversation.reply">
                                            <p class="sender_reply_p"
                                               v-if="thread.conversation.reply.message_type == 0">
                                                {{ thread.conversation.reply.message }}</p>
                                            <p class="sender_reply_p" v-else>{{ __('chat.attachment') }}</p>
                                        </div>
                                ...
                                <div class="thumb">
                                    ...
                                    <a v-else-if="user.avatar_url" href="#"><img :src="baseUrl+ user.avatar_url"
                                                                                 alt=""/></a>
                                    <a v-else href="#"><img :src="baseUrl+asset_type+'/chat/images/spondon-icon.png'"
                                                            alt=""/></a>
                                </div>
                ...
                <div v-show="replying" class="bg-gray-200 p-3 replying-box mb-2-3 ml-5">
                    <p>
                        <b class="text-info">{{ __('chat.replying') }} : </b><br/>
                    ...
                    <div class="chat_input_box d-flex align-items-center">
                        ...
                        <div class="input-group">
                            ...
                            <textarea
                                ...
                                type="text"
                                id="inputbox"
                                class="form-control auto_height"
                                :placeholder="__('chat.type') + ' '+ __('chat.message')+ '...'"
                            ...
                            <div class="input-group-append">
                                <button class="btn pr-2" title="Voice Record" type="button" @click="toggleRecording"><i
                                    :class="{'microphone-red':record_status}" class="ti-microphone-alt"></i></button>
                                <button class="btn" title="Emoji" type="button" @click="emoji = !emoji"><i
                                    :class="{'imoji-box-open':emoji}" class="ti-face-smile img_toggle"></i></button>
                                <button v-if="can_file_upload" title="Attachment" class="btn" type="button">
                                    <i class="ti-clip"></i>
                                    <input type="file" @change="onFileChange" id="imgInp" ref="file"
                                           v-on:change="onChangeFileUpload()"
                                           accept=".jpg,.jpeg,.png,.doc,.docx,.pdf,.mp4,.webm"/>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <span class="text-muted" v-if="typing">{{ __('chat.someone') }} {{ __('chat.is_typing') }}...</span>
            <div class="mt-2 bg-white timer-display d-flex justify-content-between align-item-center"
                 v-if="record_status">
                ...
                <div class="timer-padding">
                    ...
                    <span class="text-muted voice_recording">
                        {{ __('chat.your_voice_is_recording') }}...
                    </span>
                </div>
                <div class="show_record" style="display:none;">
                    <audio controls :src="audio_for_check" style="width: 500px;" class="audioCheck">
                    </audio>
                </div>
                <div class="stop-button-padding">
                    <a href="#" class="recorder_stop" @click="Stop_n_SaveRecording">
                        <img :src="baseUrl+asset_type+'/chat/images/recording-stop.png'" alt=""
                             style="height: 35px; width: 35px;"/>
                    </a>
                    <button class="recorder_delete" @click="discardRecording"><i class="fas fa-trash"></i></button>
                    <button class="svg_button send_recoder" @click="sendRecording">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                             viewBox="0 0 40 40" width="30px" height="30px">
                            <g id="surface57286370">
                                <rect x=".5" y=".5" width="40" height="40"
                                      style="fill: #FFFFFF; fill-opacity: 1; stroke: none;"/>
                                <path style="stroke: none; fill-rule: nonzero; fill: #F091FF; fill-opacity: 1;"
                                      d="M 44 7 L 4 23 L 44 39 L 37 23 Z M 36 23 L 17 23 L 35 16 L 36 22 Z M 36 23 "/>
                                <path
                                    style="stroke: none; fill-rule: nonzero; fill: #46BBFF; fill-opacity: 1;"
                        ...
                        <div class="row">
                            <div class="col-xl-12">
                                <div class="primary_input mb-15">
                                    <label class="primary_input_label">{{ __('chat.member') }}</label>
                                    <select class="primary_select mb-25" v-model="add_user" id="cash_acc_cat_id">
                                        <option value="" disabled>{{ __('chat.select') }}</option>
                                        <option v-for="single_user in connected_users" :value="single_user.id">
                                            {{ single_user.first_name }}
                                        </option>
                                    </select>
                                </div>
                                <button @click="add_new_user()" class="primary-btn radius_30px fix-gr-bg" href="#">
                                    {{ __('chat.add') }}
                                </button>
                            </div>
                        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-xl-12">
                                <div class="primary_input mb-15">
                                    <label class="primary_input_label">{{ __('chat.user_role') }}</label>
                                    <select class="primary_select mb-25" v-model="assignable_role">
                                        <option value="" disabled>{{ __('chat.select') }}</option>
                                        <option value="1">{{ __('chat.admin') }}</option>
                                        <option value="2">{{ __('chat.moderate') }}</option>
                                    </select>
                                </div>
                                <button @click="assign_role_to_user()" class="primary-btn radius_30px fix-gr-bg"
                                        href="#">{{ __('chat.add') }}
                                </button>
                            </div>
                        </div>
                                ...
                                <div v-for="single_user in group.users">
                                    <li v-if="single_user.id !== user.id" class="my-4 py-2 remove-list list-unstyled">
                                        <div class="single_list d-flex align-items-center p-2">
                                            <div class="thumb">
                                                <img class="forward-image" v-if="single_user.avatar_url"
                                                     :src="baseUrl + single_user.avatar_url" alt=""/>
                                                <a v-else href="#"><img class="forward-image"
                                                                        :src="baseUrl+single_user.avatar" alt=""/></a>
                                            </div>
                                            <div class="list_name ml-5">
                                                <a>
                                                    <h4>{{ single_user.first_name }} {{ single_user.last_name }}</h4>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="p-2">
                                            <button @click="remove_people(single_user.id)"
                                                    class="primary-btn radius_30px fix-gr-bg" href="#">
                                                {{ __('chat.remove') }}
                                            </button>
                                        </div>
                                    </li>
                        ...
                        <div class="row">
                            <div class="col-xl-12">
                                <div class="primary_input mb_20">
                                    <label class="primary_input_label" for="">{{ __('chat.keywords') }}</label>
        ...
        <div class="modal fade admin-query" id="forward" aria-modal="true">
            <div class="modal-dialog modal_800px modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">
                            {{ __('chat.forward') }} {{ __('chat.message') }}
                        </h4>
                        <button type="button" class="close" data-dismiss="modal">
                            <i class="ti-close"></i>
                        </button>
                    ...
                    <div class="modal-body" v-if="forward_conversation">
                        <div class="row">
                            <div class="col-xl-12">
                                <div class="card mb-2">
                                    <div class="card-header">"{{ this.forward_conversation.message }}"<br/></div>
                                </div>
                                <div class="primary_input mb_20">
                                    <input v-model="forward_addon_text" class="primary_input_field"
                                           :placeholder="__('chat.write_something_Optional')" type="text"/>
                                </div>
                            </div>
                            <div class="users-box">
                                <div class="col-xl-12 mt-1 mb-1" v-for="(list_user, indexFor) in all_connected_users">
                                    <div class="single_list d-flex justify-content-between">
                                        ...
                                        <div class="mt-4">
                                            ...
                                            >
                                                <i class="ti-share"></i>{{ __('chat.send') }}
                                            </a>
...
export default {
    ...
    methods: {
        Stop_n_SaveRecording() {
            this.recorder.stop();
            this.gumStream.getAudioTracks()[0].stop();
            $('.recorder_stop').hide();
            $('.recorder_delete').show();
            $('.send_recoder').show();
            $('.timer-padding').hide();
            $('.show_record').show();
            let chunks = [];
            this.recorder.ondataavailable = (e) => {
                this.halt_recording = e.data;
                chunks.push(e.data);
                const blob = new Blob(chunks, {'type': 'audio/ogg; codecs=opus'});
                chunks = [];
                const audioURL = window.URL.createObjectURL(blob);
                this.audio_for_check = audioURL;
            };
        },
        discardRecording() {
        ...
        sendRecording() {
            ...
            if (this.recorder && this.recorder.state == "recording") {
                ...
                this.recorder.ondataavailable = (e) => {
                    this.fetchRecording(e.data);
                };
            } else {
                this.record_status = false;
                this.fetchRecording(this.halt_recording);
            }
        },
        fetchRecording(data) {
            ...
            formData.append('file_attach', data);
        ...
        url_maker(text) {
            ...
            let replaceable_text = text.replace(urlRegex, function (url, b, c) {
                ...
                return '<a style="color:#19c7fb; text-decoration: underline" href="' + url2 + '" target="_blank">' + ' ' + url + '</a>';
            })

            return this.newLine(replaceable_text)
        },
