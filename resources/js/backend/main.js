(function ($) {
    ...
    $('.primary-btn').on('click', function (e) {
        ...
        // Setup
        let primaryBtnPosX = $(this).offset().left, primaryBtnPosY = $(this).offset().top,
            primaryBtnWidth = $(this).width(), primaryBtnHeight = $(this).height();
        ...
        $('.ripple')
            .css({
                width: primaryBtnWidth, height: primaryBtnHeight, top: y + 'px', left: x + 'px'
            })
    ...
    jQuery(document).ready(function ($) {
        $('.small_circle_1').circleProgress({
            value: 0.75, size: 60, lineCap: 'round', emptyFill: '#F5F7FB', thickness: '5', fill: {
    ...
    jQuery(document).ready(function ($) {
        $('.large_circle').circleProgress({
            value: 0.75, size: 228, lineCap: 'round', emptyFill: '#F5F7FB', thickness: '5', fill: {
                gradient: [["#7C32FF", .47], ["#C738D8", .3]]
            }
        });
    });
    ...
    function summernoteInit(element, selector, height = 188, placeholder = '') {
        element.summernote({
            placeholder: placeholder, tabsize: 2, height: height, tooltip: false, callbacks: {
    ...
    $('#addDeductions').on('click', function () {
        $('#addDeductionsTableBody').append('<tr>' + '<td width="80%" class="pr-30 pt-20">' + '<div class="input-effect mt-10">' + '<input class="primary-input form-control" type="text"  >' + '<label for="searchByFileName">Type</label>' + '<span class="focus-border"></span>' + '</div>' + '</td>' + '<td width="20%" class="pt-20">' + '<div class="input-effect mt-10">' + '<input class="primary-input form-control" type="text">' + '<label for="searchByFileName">Value</label>' + '<span class="focus-border"></span>' + '</div>' + '</td>' + '<td width="10%" class="pt-30">' + '<button class="primary-btn icon-only fix-gr-bg close-deductions">' + '<span class="ti-close"></span>' + '</button>' + '</td>' + '</tr>');
    });
    ...
    $('.primary-input.date').datepicker({
        autoclose: true, setDate: new Date(), rtl: RTL, language: LANG, format: $('#js_active_date_format').val()
    });
    ...
    if ($('#ckEditor').length) {
        CKEDITOR.replace('ckEditor', {
            skin: 'moono', enterMode: CKEDITOR.ENTER_BR, shiftEnterMode: CKEDITOR.ENTER_P, toolbar: [{
                name: 'basicstyles',
                groups: ['basicstyles'],
            ...
            }, {name: 'links', items: ['Link', 'Unlink']}, {name: 'insert', items: ['Image']}, {
    ...
    if ($('.active-testimonial').length) {
        $('.active-testimonial').owlCarousel({
            ...
            nav: true,
            rtl: true,
            navText: ["<img src='public/backend/img/client/prev.png' alt='' />", "<img src='public/backend/img/client/next.png'  alt='' />"]
        });
    }
    ...
    if ($('.Crm_table_active').length) {
        $('.Crm_table_active').DataTable({
            bLengthChange: false, "bDestroy": true, language: {
                paginate: {
    ...
    if ($('.Crm_table_active2').length) {
        $('.Crm_table_active2').DataTable({
            bLengthChange: false, "bDestroy": false, language: {
                search: "<i class='ti-close'></i>", searchPlaceholder: 'SEARCH HERE', paginate: {
                    next: "<i class='ti-arrow-right'></i>", previous: "<i class='ti-arrow-left'></i>"
                }
            }, columnDefs: [{
                visible: false
...
$(function () {
    $("select[multiple].active.multypol_check_select").multiselect({
        columns: 1, placeholder: "Select", search: true, searchOptions: {
            default: "Select",
        }, selectAll: true,
    });
});
...
$(document).on('click', '.removeImage', function () {
    ...
    $.ajax({
        url: url + '/admin/remove-image', type: 'POST', data: {
            table: table, name: name, id: id,
...
$(document).ready(function () {
    const activeMenu = $('#sidebar_menu').find('.mm-active').offset().top;
    const mainMenu = $('#sidebar_menu').offset().top;
