                    ...
                    <div class="col-lg-6 col-md-6 mt_20">
                        <div class="single_input ">
                            ...
                            <select class="theme_select wide mb_20"
                                    name="account_type" {{$errors->first('account_type') ? 'autofocus' : ''}} id="account_type">
                                <option value=""> {{__('profile.select_one')}}</option>
                                @foreach($payout_accounts as $payout_account)
                                    <option {{@$user->userPayoutAccount->payout_accounts_id == $payout_account->id ?'selected':""}} value="{{$payout_account->id}}">{{$payout_account->title}}</option>
                                @endforeach
                            </select>
