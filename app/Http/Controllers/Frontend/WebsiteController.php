...
class WebsiteController extends Controller
{
        ...
        try {
            ...
            if (isModuleActive('Subscription')) {
                if (Auth::check() && !isSubscriptionExpire()) {
                    Toastr::error('Your subscription validity expired', 'Access Denied');
                    return redirect()->back();
                }
            }
...

    {
        try {
            ...
            if ($course->isLoginUserEnrolled) {
                if ($course->type == 5) {
                    Toastr::error(trans('product.Product already enrolled'), 'Failed');
                } else {
                ...
                foreach ($items as $key => $cart) {
                    ...
                    if ($course) {
                        ...
                        if (isModuleActive('Installment') && $cart->is_installment == 1) {
                            $carts[$key]['price'] = getPriceFormat(installmentProductPrice($cart->course_id, $cart->plan_id));
        ...
        try {
            ...
            if ($course) {
                ...
                if ($percentage >= 100 && $newLessonComplete) {
                    ...
                    if (UserSmsNotificationSetup('Complete_Course', $user)) {
                        send_sms_notification($user, 'Complete_Course', [
                            'time' => Carbon::now()->format('d-M-Y, g:i A'),
                            'course' => $course->title
                        ]);
                    }
...
//    public function bundleSubscriptionCourses()
//    {
//        if (!Auth::check()) {
//            return redirect('login');
//        }
//        if (!isModuleActive('BundleSubscription')) {
//            Toastr::error('Bundle Subscription Module not active', 'Error');
//            return redirect()->back();
//        }
//        try {
//            return view(theme('pages.bundle-subscription-courses'));
//        } catch (\Exception $e) {
//            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
//        }
//    }
