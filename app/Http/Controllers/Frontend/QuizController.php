...
class QuizController extends Controller
{
    ...
    public function quizDetails($slug, $id, Request $request)
    {


        try {
            $realId = substr($id, 0, -8);
            $onlineQuiz = OnlineQuiz::find($id);            
            $select_field = [
                'courses.id',
                'courses.type',
                'courses.slug',
            ...
            $course->save();
            return view(theme('pages.quizDetails'), compact('realId', 'course', 'request', 'isEnrolled'));
