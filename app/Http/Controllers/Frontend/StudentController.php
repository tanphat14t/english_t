...
{
    use ImageStore, GoogleAnalytics4;
    ...
    public function myQuizzesDetail($id){
        try {
            return view(theme('pages.myQuizzesDetail'), compact('id'));
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }
...
//            ]);
    ...
    public function getCertificate($id, $slug, Request $request)
    {
        ...
        try {
            ...
            $request->certificate_id = $course->certificate_id;
        ...
        try {
            ...
            if (is_null($review)) {
                ...
                if (UserBrowserNotificationSetup('Course_Review', $course->user)) {
                    send_browser_notification($course->user, 'Course_Review', [
                        'time' => Carbon::now()->format('d-M-Y, g:i A'),
                        'course' => $course->title,
                        'review' => $newReview->comment,
                        'star' => $newReview->star,
                ...
                if (UserSmsNotificationSetup('Course_Review', $course->user)) {
                    send_sms_notification($course->user, 'Course_Review', [
                        'time' => Carbon::now()->format('d-M-Y, g:i A'),
                        'course' => $course->title,
                        'review' => $newReview->comment,
                        'star' => $newReview->star,
    ...
    public function flashcardFnc(){
        try {
            return view(theme('pages.myFlashcard'));
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }

    public function flashcardDetailFnc($id){
        try {
            $id = substr($id, 0, -8);
            return view(theme('pages.myFlashcardDetail'), compact('id'));
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }
