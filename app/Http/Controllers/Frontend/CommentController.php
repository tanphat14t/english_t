    ...
    public function saveComment(Request $request)
    {
        ...
        try {
            ...
            if (isset($course)) {
                ...
                if (UserSmsNotificationSetup('Course_comment', $courseUser)) {
                    send_sms_notification($courseUser, 'Course_comment', [
                        'time' => Carbon::now()->format('d-M-Y, g:i A'),
                        'course' => $course->title,
                        'comment' => $comment->comment,
                    ]);
            ...
            if (isset($course)) {
                ...
                if ($course->user->id != Auth::user()->id) {
                    ...
                    if (UserSmsNotificationSetup('Course_comment_Reply', $course->user)) {
                        send_sms_notification($course->user, 'Course_comment_Reply', [
                                'time' => Carbon::now()->format('d-M-Y, g:i A'),
                                'course' => $course->title,
                                'comment' => $comment->comment,
                                'reply' => $comment->reply,
                ...
                if (UserSmsNotificationSetup('Course_comment_Reply', $commentUser)) {
                    send_sms_notification($commentUser, 'Course_comment_Reply', [
                        'time' => Carbon::now()->format('d-M-Y, g:i A'),
                        'course' => $course->title,
                        'comment' => $comment->comment,
                        'reply' => $comment->reply,
