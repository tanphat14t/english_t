...
//           'reason'=>'required',
        ...
        if (UserSmsNotificationSetup($act, $enroll->user)) {
            send_sms_notification($enroll->user, $type = $act, $shortcodes = [
                'course' => $enroll->course->title,
                'time' => now(),
                'reason' => ''
            ]
        ...
        if (UserSmsNotificationSetup($act, $enroll->course->user)) {
            send_sms_notification($enroll->course->user, $type = $act, $shortcodes = [
                'course' => $enroll->course->title,
                'time' => now(),
                'reason' => ''
            ]);
    ...
    {
        $user = Auth::user();
        $query = CourseEnrolled::select('course_enrolleds.*')->with('user', 'course');
