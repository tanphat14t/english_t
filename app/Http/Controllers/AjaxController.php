    ...
    public function statusEnableDisable(Request $request)
    {
        ...
        try {
            ...
            if ($table == "courses") {
                ...
                if ($course->type == 1) {
                    if ($status == 1) {
                        $email_template_act = 'Course_Publish_Successfully';
                        $topic_title_key = 'course';
                    } else {
                        $email_template_act = 'Course_Unpublished';
                        $topic_title_key = 'course';
                    }
                } elseif ($course->type == 2) {
                    if ($status == 1) {
                        $email_template_act = 'Quiz_Publish_Successfully';
                        $topic_title_key = 'quiz';
                    } else {
                        $email_template_act = 'Quiz_Unpublished';
                        $topic_title_key = 'quiz';
                    }
                }
                ...
                if (in_array($course->type, [1, 2])) {
                    if ($status == 1) {
                        ...
                        if (UserMobileNotificationSetup($email_template_act, $course->user) && !empty($course->user->device_token)) {
                            send_mobile_notification($course->user, $email_template_act, [
                                'time' => Carbon::now()->format('d-M-Y, g:i A'),
                                $topic_title_key => $course->title,
                            ]);
                        }

                        if (UserEmailNotificationSetup($email_template_act, $course->user)) {
                            SendGeneralEmail::dispatch($course->user, $email_template_act, [
                                'time' => Carbon::now()->format('d-M-Y, g:i A'),
                                $topic_title_key => $course->title,
                            ]);
                        }
                        if (UserBrowserNotificationSetup($email_template_act, $course->user)) {

                            send_browser_notification($course->user, $type = $email_template_act, $shortcodes = [
                                'time' => Carbon::now()->format('d-M-Y, g:i A'),
                                $topic_title_key => $course->title,
                            ],
                        ...
                        if (UserSmsNotificationSetup($email_template_act, $course->user)) {

                            send_sms_notification($course->user, $type = $email_template_act, $shortcodes = [
                                'time' => Carbon::now()->format('d-M-Y, g:i A'),
                                $topic_title_key => $course->title,
                            ]);
                        }
