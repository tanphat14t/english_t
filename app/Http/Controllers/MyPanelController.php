...
class MyPanelController extends Controller
{
    ...
    {
        try {
            $data = RefundRequest::with('refund_details', 'refund_details.refund_products', 'order')->where('customer_id', auth()->user()->id)->latest();
            return DataTables::of($data)
                ...
                ->editColumn('request_sent_date', function ($row) {
        ...
        try {
            ...
            foreach ($data['product_ids'] as $key => $send_product_id) {
                ...
                $request_product_info = [
                    ...
                    'refund_reason_id' => $data['reason_' . $split[4]],
                    'return_qty' => $split[1],
                    'return_amount' => $amount[$key] * $split[1],
                ];
