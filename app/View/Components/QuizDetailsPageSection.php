...
{
    public $course, $request, $isEnrolled, $id;

    public function __construct($course, $request, $isEnrolled, $id)
    {
        $this->course = $course;
    ...
    public function render()
    {
        ...
        $givenQuiz = QuizTest::where('user_id', Auth::id())->where('course_id', $this->course->id)->where('quiz_id', $this->id)->get();
        ...
        $currentQuiz = OnlineQuiz::find($this->id);
        return view(theme('components.quiz-details-page-section'), compact('currentQuiz', 'preResult', 'isPass', 'levels', 'related', 'certificate', 'alreadyJoin', 'is_cart', 'isFree', 'isBookmarked', 'quizSetup', 'reviewer_user_ids'));
    }
}
