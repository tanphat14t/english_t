...
class OfflinePaymentController extends Controller
{
    ...
    public function addBalance(Request $request)
    {
        ...
        try {
            ...
            if (UserSmsNotificationSetup('OffLine_Payment', $user)) {

                send_sms_notification($user, 'OffLine_Payment', [
                        'amount' => $request->amount,
                        'currency' => Settings('currency_code'),
                        'time' => now()->format(Settings('active_date_format') . ' H:i:s A'),
                    ]
    ...
    public function deductBalance(Request $request)
    {
        ...
        try {
            ...
            if (UserSmsNotificationSetup('Deduct_Payment', $user)) {

                send_sms_notification($user, 'Deduct_Payment', [
                    'amount' => getPriceFormat($request->amount),
                    'time' => now()->format(Settings('active_date_format') . ' H:i:s A'),
                ]);
            }
