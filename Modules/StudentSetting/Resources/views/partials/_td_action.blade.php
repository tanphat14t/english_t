<div class="dropdown CRM_dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2">
        ...
        <a class="dropdown-item" href="{{ route('student.show', $query->id) }}">
            {{ trans('common.Show') }}</a>

        <a href="{{ route('student.edit', $query->id) }}" class="dropdown-item "
            type="button">{{ trans('common.Edit') }}
        </a>

        @if (permissionCheck('student.delete'))
            <button class="dropdown-item deleteStudent" data-id="{{ $query->id }}"
                type="button">{{ trans('common.Delete') }}
            </button>
        @endif

        @if (permissionCheck('student.courses'))
            <a class="dropdown-item" href="{{ route('student.courses', $query->id) }}" data-id="{{ $query->id }}"
                type="button">{{ trans('courses.Course') }}</a>
        @endif
