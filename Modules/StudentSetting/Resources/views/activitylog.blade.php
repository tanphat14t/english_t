                                    ...
                                    @if(isset($logins))
                                        @foreach ($logins as $key=> $login)
                                            <tr>
                                                ...
                                                <td>{{$login->os}}</td>
                                                <td>{{$login->browser}}</td>
                                                <td>{{ $login->logout_at ? $login->logout_at : trans('common.Active') }}</td>
                                                <td>
                                                    @php
                                                        $ip = request()->ip();
                                                    ...
                                                    @if(empty($login->logout_at))
                                                        <a href="#" data-toggle="modal"
                                                           data-target="#logOut" data-id="{{$login->id}}"
                                                           class="primary-btn radius_30px mr-10 fix-gr-bg text-white logoutBtn">{{__('frontend.Logout from system')}}</a>
                                                    @endif
                                                </td>
