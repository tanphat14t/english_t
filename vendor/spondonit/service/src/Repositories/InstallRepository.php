...
class InstallRepository
{
    ...
    public function getPreRequisite()
    {
        ...
        $server[] = $this->check($this->my_version_compare(phpversion(), config('spondonit.php_version', '8.2.0'), '>='),'Current Version ' . phpversion(), sprintf('Min PHP version ' . config('spondonit.php_version', '8.2.0') . ' (%s)', 'Current PHP Version ' . phpversion()), true);
    ...
    public function validateLicense($params)
    {
        ...
        $url = verifyUrl(config('spondonit.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . request('access_code') . '&i=' . config('app.item') . '&e=' . request('envato_email') . '&ri=' . request('re_install') . '&current=' . urlencode(request()->path()) . '&ve=' . $ve . '&v=' . $v.'&enc=1';
    ...
    public function installModule($params)
    {
        ...
        $url = verifyUrl($verifier) . '/api/cc?a=install&u=' . app_url() . '&ac=' . $code . '&i=' . $item_id . '&e=' . $e . '&t=Module&ve=' . $ve . '&name=' . $name . '&row=' . $row . '&file=' . $file.'&current='.str_replace(url('/').'/', '', url()->previous().'&enc=1');
    ...
    public function installTheme($params)
    {
        ...
        $url = verifyUrl(config('spondonit.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . $code . '&i=' . $item_id . '&e=' . $e . '&t=Theme&enc=1';
